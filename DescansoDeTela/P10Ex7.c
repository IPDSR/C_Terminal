...
int main(int argc, char **argv){
   ...
   al_clear_to_color(al_map_rgb(255, 255, 255)); //		COLOQUEI COR BRANCA NO BOUNCER
   ...
   while(1)
   {
      ...
      if(ev.type == ALLEGRO_EVENT_TIMER) {
         ...
         if(bouncer_x < 0 || bouncer_x > SCREEN_W - BOUNCER_SIZE) {
			...
			al_set_target_bitmap(bouncer);	//AVISA QUE VOU MUDAR O BOUNCER
			al_clear_to_color(al_map_rgb(255, 255, 0));	//MUDA PARA COR AMARELA QUANDO ENCOSTA NA PAREDE
			al_set_target_bitmap(al_get_backbuffer(display));
		 }
         ...
         if(bouncer_y < 0 || bouncer_y > SCREEN_H - BOUNCER_SIZE) {
			...
			al_set_target_bitmap(bouncer);	//AVISA QUE VOU MUDAR O BOUNCER
			al_clear_to_color(al_map_rgb(255, 0, 255));	//MUDA PARA COR PRETA QUANDO ENCOSTA NO TETO OU NO CHAO
			al_set_target_bitmap(al_get_backbuffer(display));
         }
